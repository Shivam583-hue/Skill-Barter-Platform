    ...
    const fetch = async () => {
      try {
        const response = await axios.get(
          `${baseUrl}/api/groupsJoinedbyAuthenticatedUser/${authUser?.id}`,
        );
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setTimeout(
          () =>
            alert(
              `${response.data.data.groupId} is the groupId, keep it secure and remember it for future use cases`,
            ),
          2000,
        );
      } else {
  ...
  return (
    <div className="flex flex-col mt-5 items-center h-screen">
      <div className="w-max h-max mb-5 p-1 rounded-full bg-[#121212] border border-white shadow-lg flex flex-row">
        <motion.button
          className={`${isHomeActive ? "bg-[#2f3030] text-white" : "bg-transparent text-white"} px-6 py-2 mx-1 rounded-full transition duration-300 ease-in-out hover:bg-[#2f3030] hover:text-white`}
          onClick={() => {
        ...
        <motion.button
          className={`${isChatroomsActive ? "bg-[#2f3030] text-white" : "bg-transparent text-white"} px-6 py-2 mx-1 rounded-full transition duration-300 ease-in-out hover:bg-[#2f3030] hover:text-white`}
          onClick={() => {
      ...
      <motion.button
        onClick={() => setToggleDialogue(true)}
        whileHover={{ scale: 1.1 }}
        whileTap={{ scale: 1 }}
        className="absolute font bottom-10 right-10 bg-black border border-white text-white p-3 rounded-full border-2 shadow-lg z-50 hover:bg-gray-900 focus:outline-none"
      >
        <svg
      ...
      <Dialog open={toggleDiaglogue} onOpenChange={setToggleDialogue}>
        <DialogContent className="bg-black text-white font-mono">
          ...
          <div>
            <p className="text-gray-100 mb-5 text-sm">
              Press the create button once and then wait for a success message.
            </p>
